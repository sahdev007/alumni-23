...
export class NewsAndUpdatesComponent implements OnInit {
  ...
  public columnsToDisplay: string[] = ['sr.no' ,'newsImage', ...this.displayedColumns, 'description', 'status', 'actions'];
  ...
  async getAllData() {
    ...
    await this.dataService.getAllData(action).subscribe(
      (res: any) => {
        ...
        if(res?.status == 200) this.allNews = res?.data; this.dataSource.data = res?.data;
      },
      (error) => {
        this.notifyService.notificationService.error(error);
      }
