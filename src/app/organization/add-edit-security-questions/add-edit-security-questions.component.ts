...
@Component({
  selector: 'app-add-edit-security-questions',
  templateUrl: './add-edit-security-questions.component.html',
  styleUrls: ['./add-edit-security-questions.component.scss']
})
export class AddEditSecurityQuestionsComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    /**Get and single team data and Patch with form */
      if(this.eventId) this.getSingleQuestion();
  }

  async getSingleQuestion() {
    this.loading = true;
    let action: string = "single-questions";
   ...
   */
    async save() {
      ...
      if(this.form?.invalid){
        ...
        let action: string = this.pageType == "update-questions" ? 'update-questions': 'create-questions';
        await this.organizationServie.postData(action, this.form?.value).subscribe((item: any) => {
          if(item?.status == 200){
            ...
            this.router.navigate(['/organization/security-questions']);
          }
        }, error => {
