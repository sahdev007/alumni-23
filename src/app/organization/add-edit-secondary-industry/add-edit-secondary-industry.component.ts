...
@Component({
  selector: 'app-add-edit-secondary-industry',
  templateUrl: './add-edit-secondary-industry.component.html',
  styleUrls: ['./add-edit-secondary-industry.component.scss']
})
export class AddEditSecondaryIndustryComponent implements OnInit {
  ...
  async getSingleIndustry() {
    this.loading = true;
    let action: string = "single-secondaryIndustry";
   ...
   */
    async save() {
      ...
      if(this.form?.invalid){
        ...
        let action: string = this.pageType == "update-secondaryIndustry" ? 'update-secondaryIndustry': 'create-secondaryIndustry';
        await this.organizationServie.postData(action, this.form?.value).subscribe((item: any) => {
          if(item?.status == 200){
            ...
            this.router.navigate(['/organization/secondary-industry']);
          }
        }, error => {
