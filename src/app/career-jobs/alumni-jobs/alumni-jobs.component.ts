...
export class AlumniJobsComponent implements OnInit {
  ...
  add() {
    this.router.navigate(["/careers-jobs/add-job"]);
  }

  view(data: any) {
    console.log(data)
    this.router.navigate(['/careers-jobs/view-job'], {queryParams: { id: data?.id, type: "alumni-jobs" }});
  }
    ...
    await this.collaborateService.getAllData(action).subscribe(
      (res: any) => {
        if (res?.status == 200) {
          ...
          this.alumniJobs = this.dataSource?.data;
        }
      },
      (error) => {
          this.notify.notificationService.error(error);
